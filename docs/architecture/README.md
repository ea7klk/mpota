...
- Activations are grouped by activator, park, and UTC date. A group with fewer than 10 valid QSOs remains visible as `FAILED` and gives the activator no credit; valid hunter QSOs from that group still count for hunters. Global administrators can search and delete an exact activation's QSOs through the audited administration API.
...
- Kubernetes production bootstraps a prebuilt worldwide MBTiles artifact into the `mpota-tiles` PVC. The optional self-hosted Compose profile downloads the Europe OSM PBF extract with 5% progress logging, converts it to OpenMapTiles-compatible MBTiles with tilemaker with 5% extraction progress logging, and stores the result in the named volume `mpota-tile-data`. Both self-hosted paths use atomic replacement and an explicit force-refresh flag.
