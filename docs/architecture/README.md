...
- Activations are grouped by activator, park, and UTC date. A group with fewer than 5 valid QSOs remains visible as `FAILED` and gives the activator no credit; valid hunter QSOs from that group still count for hunters. Global administrators can search and delete an exact activation's QSOs through the audited administration API.
