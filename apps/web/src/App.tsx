...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main>{tab === 'park-editor' ? null : tab === 'home' ? <HomePage parks={parks} t={t} locale={locale} token={token} onMap={() => setTab('map')} onPropose={() => setTab('propose')} onQso={() => setTab('qso')} onSignIn={() => setAuthMode('login')} /> : <><section className="hero"><div className="hero-copy"><p className="eyebrow">Municipal Parks on the Air</p><h1>{t.hero}</h1><p className="hero-text">{t.heroText}</p><div className="hero-actions"><button className="button" onClick={() => setTab('propose')}>{t.propose}</button><span className="stat"><strong>{parks.length}</strong> {t.approved}</span></div></div></section></>}
