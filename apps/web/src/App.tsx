...
function MapView({ parks, picking, selectedPoint, view, onPick, onViewChange, onParkSelect }: { parks: Park[]; picking: boolean; selectedPoint?: Coordinates; view: MapViewState; onPick: (lat: number, lon: number) => void; onViewChange: (view: MapViewState) => void; onParkSelect: (park: Park) => void }) {
  ...
  useEffect(() => { pickingRef.current = picking; onPickRef.current = onPick; onViewChangeRef.current = onViewChange; onParkSelectRef.current = onParkSelect; }, [picking, onPick, onViewChange, onParkSelect]);
  useEffect(() => {
