...
type Role = 'MEMBER' | 'ENTITY_ADMIN' | 'AWARD_ADMIN' | 'GLOBAL_ADMIN' | 'SYSTEM_ADMIN' | 'SYSTEM_BOOTSTRAP_ADMIN';
...
type ParkTypeSetting = { code: string; labels: Record<string, string>; active: boolean; sortOrder: number };
type DatabaseSystemSettings = { parkTypes: ParkTypeSetting[]; translations: Partial<Record<Locale, Partial<TranslationCatalog>>> };
...
const ROLE_OPTIONS: Role[] = ['MEMBER', 'ENTITY_ADMIN', 'AWARD_ADMIN', 'GLOBAL_ADMIN', 'SYSTEM_ADMIN', 'SYSTEM_BOOTSTRAP_ADMIN'];
const DEFAULT_PARK_TYPES: ParkTypeSetting[] = [
...
function parkTypeLabel(code: string, parkTypes: ParkTypeSetting[], locale: Locale, fallback: string) {
  const type = parkTypes.find((candidate) => candidate.code === code);
  return type?.labels[locale] || type?.labels.en || fallback || code.replaceAll('_', ' ');
}

function mergeCatalog(base: TranslationCatalog, override?: Partial<TranslationCatalog>): TranslationCatalog {
  if (!override) return base;
  return {
    ...base,
    ...override,
    copy: { ...base.copy, ...(override.copy ?? {}) },
    homeEligibility: { ...base.homeEligibility, ...(override.homeEligibility ?? {}) },
    policies: {
      ...base.policies,
      ...(override.policies ?? {}),
      conduct: { ...base.policies.conduct, ...(override.policies?.conduct ?? {}) },
      rules: { ...base.policies.rules, ...(override.policies?.rules ?? {}) }
    }
  };
}
...
async function request<T>(path: string, options: RequestInit = {}, token?: string): Promise<T> {
  ...
  const response = await fetch(`${API}${requestPath}`, { ...options, cache: method === 'GET' ? 'no-store' : options.cache, headers });
  const body = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(Array.isArray(body.message) ? body.message.join(', ') : body.message ?? 'Request failed');
  return body as T;
}

function SiteTopbar({ nav, adminMenu, activeTab, locale, user, t, onLocaleChange, onNavigate, onSignIn, onRegister, onSignOut }: { nav: NavItem[]; adminMenu?: NavItem[]; activeTab: string; locale: Locale; user: User | null; t: Record<string, string>; onLocaleChange: (locale: Locale) => void; onNavigate: (id: string) => void; onSignIn: () => void; onRegister: () => void; onSignOut: () => void }) {
  return <header className="topbar"><div className="brand"><img className="header-banner" src="/mpota-logo-banner.svg" alt="MPOTA — Municipal Parks On The Air" /></div><nav>{nav.map((item) => <button className={(activeTab === item.id || (activeTab === 'park-editor' && item.id === 'park-admin')) ? 'nav-active' : ''} key={item.id} onClick={() => onNavigate(item.id)}>{item.label}</button>)}{adminMenu?.length ? <details className="admin-menu"><summary>{t.adminMenu || 'Admin'}</summary><div className="admin-menu-items">{adminMenu.map((item) => <button className={activeTab === item.id ? 'nav-active' : ''} key={item.id} onClick={() => onNavigate(item.id)}>{item.label}</button>)}</div></details> : null}</nav><div className="header-actions"><select aria-label={t.language} value={locale} onChange={(event) => onLocaleChange(event.target.value as Locale)}><option value="en">EN</option><option value="es">ES</option><option value="fr">FR</option><option value="de">DE</option></select>{user ? <><button className="user-pill" onClick={() => onNavigate('profile')}>{user.callsign || user.displayName}</button><button className="button ghost" onClick={onSignOut}>{t.signOut}</button></> : <><button className="button ghost" onClick={onSignIn}>{t.signIn}</button><button className="button" onClick={onRegister}>{t.register}</button></>}</div></header>;
}
...
function MapView({ parks, picking, selectedPoint, view, t, onPick, onViewChange, onParkSelect, onVisibleParksChange, onViewportChange, onLocationError }: { parks: Park[]; picking: boolean; selectedPoint?: Coordinates; view: MapViewState; t: Record<string, string>; onPick: (lat: number, lon: number) => void; onViewChange: (view: MapViewState) => void; onParkSelect: (park: Park) => void; onVisibleParksChange: (parks: Park[]) => void; onViewportChange?: (bounds: L.LatLngBounds) => void; onLocationError?: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    const handleViewportChange = () => { saveView(); updateVisibleParks(); onViewportChangeRef.current?.(map.current!.getBounds()); };
    map.current.on('moveend zoomend', handleViewportChange);
    updateVisibleParks();
...
function ParkEditorPage({ parkId, token, t, parkTypes, locale, onClose, onSaved }: { parkId: string; token: string; t: Record<string, string>; parkTypes: ParkTypeSetting[]; locale: Locale; onClose: () => void; onSaved: () => void }) {
...
function ParkDetailsModal({ park, token, parkTypes, locale, onClose, onMessage }: { park: Park; token: string; parkTypes: ParkTypeSetting[]; locale: Locale; onClose: () => void; onMessage: (message: string) => void }) {
...
function ParkDetailPage({ reference, token, t, parkTypes, adminMenu, onBack, onMessage, nav, activeTab, locale, user, onLocaleChange, onNavigate, onSignIn, onRegister, onSignOut }: { reference: string; token: string; t: Record<string, string>; parkTypes: ParkTypeSetting[]; adminMenu: NavItem[]; onBack: () => void; onMessage: (message: string) => void; nav: NavItem[]; activeTab: string; locale: Locale; user: User | null; onLocaleChange: (locale: Locale) => void; onNavigate: (id: string) => void; onSignIn: () => void; onRegister: () => void; onSignOut: () => void }) {
  ...
  const detailTopbar = <><SiteTopbar nav={nav} adminMenu={adminMenu} activeTab={activeTab} locale={locale} user={user} t={t} onLocaleChange={onLocaleChange} onNavigate={onNavigate} onSignIn={onSignIn} onRegister={onRegister} onSignOut={onSignOut} /><div className="detail-actionbar"><button className="button ghost" onClick={onBack}>← {t.back}</button></div></>;
...
function PolicyPage({ kind, catalog, onBack }: { kind: PolicyKind; catalog: TranslationCatalog; onBack: () => void }) {
  const page = catalog.policies[kind];
...
function HomePage({ parks, t, catalog, token, onMap, onPropose, onQso, onSignIn, onPolicy }: { parks: Park[]; t: Record<string, string>; catalog: TranslationCatalog; token: string; onMap: () => void; onPropose: () => void; onQso: () => void; onSignIn: () => void; onPolicy: (kind: PolicyKind) => void }) {
  const eligibility = catalog.homeEligibility;
  const conduct = catalog.policies.conduct;
  const rules = catalog.policies.rules;
...
function setNestedTranslationValue(target: Record<string, unknown>, path: string, value: string) {
  const parts = path.replace(/\[(\d+)\]/g, '.$1').split('.').filter(Boolean);
  let cursor: any = target;
  parts.forEach((part, index) => {
    if (index === parts.length - 1) { cursor[part] = value; return; }
    if (cursor[part] === undefined || cursor[part] === null) cursor[part] = /^\d+$/.test(parts[index + 1]) ? [] : {};
    cursor = cursor[part];
  });
}

function SystemSettingsPage({ token, locale, t, settings, onSaved, onMessage }: { token: string; locale: Locale; t: Record<string, string>; settings: DatabaseSystemSettings; onSaved: (settings: DatabaseSystemSettings) => void; onMessage: (message: string) => void }) {
  const [selectedLocale, setSelectedLocale] = useState<Locale>(locale);
  const [search, setSearch] = useState('');
  const [parkTypes, setParkTypes] = useState<ParkTypeSetting[]>(settings.parkTypes.length ? settings.parkTypes : DEFAULT_PARK_TYPES);
  const [drafts, setDrafts] = useState<Record<Locale, TranslationCatalog>>(() => ({
    en: mergeCatalog(catalogs.en, settings.translations.en),
    es: mergeCatalog(catalogs.es, settings.translations.es),
    fr: mergeCatalog(catalogs.fr, settings.translations.fr),
    de: mergeCatalog(catalogs.de, settings.translations.de)
  }));
  const [saving, setSaving] = useState(false);
  useEffect(() => {
    setParkTypes(settings.parkTypes.length ? settings.parkTypes : DEFAULT_PARK_TYPES);
    setDrafts({ en: mergeCatalog(catalogs.en, settings.translations.en), es: mergeCatalog(catalogs.es, settings.translations.es), fr: mergeCatalog(catalogs.fr, settings.translations.fr), de: mergeCatalog(catalogs.de, settings.translations.de) });
  }, [settings]);
  const sourceKeys = useMemo(() => Object.keys(flattenTranslations(catalogs.en)), []);
  const currentDraft = drafts[selectedLocale] ?? catalogs[selectedLocale];
  const currentValues = flattenTranslations(currentDraft);
  const filteredKeys = sourceKeys.filter((key) => !search.trim() || `${key} ${currentValues[key] || ''}`.toLowerCase().includes(search.toLowerCase()));
  const updateTranslation = (key: string, value: string) => {
    const next = JSON.parse(JSON.stringify(currentDraft)) as TranslationCatalog;
    setNestedTranslationValue(next as unknown as Record<string, unknown>, key, value);
    setDrafts((current) => ({ ...current, [selectedLocale]: next }));
  };
  const updateParkType = (index: number, patch: Partial<ParkTypeSetting>) => setParkTypes((current) => current.map((type, typeIndex) => typeIndex === index ? { ...type, ...patch } : type));
  const updateParkLabel = (index: number, language: Locale, label: string) => setParkTypes((current) => current.map((type, typeIndex) => typeIndex === index ? { ...type, labels: { ...type.labels, [language]: label } } : type));
  const addParkType = () => setParkTypes((current) => [...current, { code: `NEW_TYPE_${current.length + 1}`, labels: { en: 'New park type' }, active: true, sortOrder: (current.length + 1) * 10 }]);
  const save = async () => {
    setSaving(true);
    try {
      const updated = await request<DatabaseSystemSettings>('/admin/settings', { method: 'PATCH', body: JSON.stringify({ parkTypes, translations: drafts }) }, token);
      onSaved(updated);
      onMessage('System settings saved.');
    } catch (error) { onMessage((error as Error).message); }
    finally { setSaving(false); }
  };
  return <section className="system-settings-page"><div className="single-panel panel"><div className="section-heading"><div><p className="eyebrow">{t.globalAdministration}</p><h1>{t.systemSettings}</h1><p className="muted">{t.systemSettingsDescription}</p></div><button className="button" type="button" onClick={() => void save()} disabled={saving}>{saving ? t.saving : t.save}</button></div><section className="settings-section"><div className="section-heading"><div><p className="eyebrow">{t.parkTypes}</p><h2>{t.parkTypes}</h2><p className="muted">{t.parkTypesDescription}</p></div><button className="button ghost" type="button" onClick={addParkType}>{t.addParkType}</button></div><div className="park-type-settings">{parkTypes.map((type, index) => <article className="park-type-row" key={`${type.code}-${index}`}><div className="park-type-fields"><label>{t.code}<input value={type.code} onChange={(event) => updateParkType(index, { code: event.target.value.toUpperCase().replace(/[^A-Z0-9_]/g, '_') })} /></label><label>{t.sortOrder}<input type="number" value={type.sortOrder} onChange={(event) => updateParkType(index, { sortOrder: Number(event.target.value) })} /></label><label className="checkbox-label"><input type="checkbox" checked={type.active} onChange={(event) => updateParkType(index, { active: event.target.checked })} /> {t.active}</label></div><div className="park-type-labels">{(['en', 'es', 'fr', 'de'] as Locale[]).map((language) => <label key={language}>{language.toUpperCase()}<input value={type.labels[language] || ''} onChange={(event) => updateParkLabel(index, language, event.target.value)} /></label>)}</div></article>)}</div></section><section className="settings-section"><div className="section-heading"><div><p className="eyebrow">{t.translations}</p><h2>{t.translationMaintenance}</h2><p className="muted">{t.databaseTranslationsDescription}</p></div><span className="badge">{sourceKeys.length} keys</span></div><div className="translation-toolbar"><label>{t.language}<select value={selectedLocale} onChange={(event) => setSelectedLocale(event.target.value as Locale)}><option value="en">English</option><option value="es">Español</option><option value="fr">Français</option><option value="de">Deutsch</option></select></label><label>{t.search}<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t.searchTranslations} /></label></div><div className="translation-list">{filteredKeys.map((key) => <label className="translation-row" key={key}><span>{key}</span><textarea rows={2} value={currentValues[key] ?? ''} onChange={(event) => updateTranslation(key, event.target.value)} /></label>)}</div></section></div></section>;
}
...
export default function App() {
  ...
  const [mapParks, setMapParks] = useState<Park[]>([]);
  ...
  const [systemSettings, setSystemSettings] = useState<DatabaseSystemSettings>({ parkTypes: DEFAULT_PARK_TYPES, translations: {} });
  ...
  const [message, setMessage] = useState('');
  const activeCatalog = useMemo(() => mergeCatalog(catalogs[locale], systemSettings.translations[locale]), [locale, systemSettings]);
  const t = activeCatalog.copy;
  const parkTypes = systemSettings.parkTypes.length ? systemSettings.parkTypes : DEFAULT_PARK_TYPES;
  const canSystemAdmin = Boolean(user && ['GLOBAL_ADMIN', 'SYSTEM_ADMIN', 'SYSTEM_BOOTSTRAP_ADMIN'].includes(user.role));
  const canModerate = Boolean(user && ['ENTITY_ADMIN', 'GLOBAL_ADMIN', 'SYSTEM_BOOTSTRAP_ADMIN'].includes(user.role));
  const canManageUsers = canSystemAdmin;
  const canAward = Boolean(user && ['AWARD_ADMIN', 'GLOBAL_ADMIN', 'SYSTEM_ADMIN', 'SYSTEM_BOOTSTRAP_ADMIN'].includes(user.role));
  const canGlobalAdmin = canSystemAdmin;
  const qsoAdminLabel = t.qsoAdmin;

  const refreshParks = (bounds?: L.LatLngBounds) => bounds ? refreshMapParks(bounds) : request<Park[]>('/parks').then(setParks).catch(() => undefined);
  const refreshMapParks = (bounds: L.LatLngBounds) => {
    const requestId = ++mapRequest.current;
    const params = new URLSearchParams({ south: String(bounds.getSouth()), north: String(bounds.getNorth()), west: String(bounds.getWest()), east: String(bounds.getEast()) });
    request<Park[]>('/parks/viewport?' + params.toString()).then((result) => { if (requestId === mapRequest.current) setMapParks(result); }).catch(() => undefined);
  };
  const refresh = () => { void refreshParks(); request<Award[]>('/awards').then(setAwards).catch(() => undefined); request<DatabaseSystemSettings>('/settings/public').then(setSystemSettings).catch(() => undefined); if (token) request<User>('/auth/me', {}, token).then(setUser).catch(() => { setToken(''); localStorage.removeItem('mpota-token'); }); };
  ...
  const nav = useMemo(() => [{ id: 'home', label: t.home }, { id: 'map', label: t.map }, ...(user ? [{ id: 'propose', label: t.propose }, { id: 'qso', label: t.qso }, { id: 'uploads', label: t.uploads }] : []), { id: 'awards', label: t.awards }, ...(canModerate ? [{ id: 'admin', label: t.admin }, { id: 'park-admin', label: t.parkAdmin }] : [])], [t, user, canModerate]);
  const adminMenu = useMemo(() => canSystemAdmin ? [{ id: 'qso-admin', label: qsoAdminLabel }, { id: 'awards', label: t.awards }, { id: 'translations', label: t.translations }, { id: 'users', label: t.users }, { id: 'system-settings', label: t.systemSettings }] : [], [canSystemAdmin, qsoAdminLabel, t]);
  ...
  return <div className="app-shell">
    ...
    <main>{tab === 'park-editor' ? null : tab === 'home' ? <HomePage parks={parks} t={t} catalog={activeCatalog} token={token} onMap={() => setTab('map')} onPropose={() => setTab('propose')} onQso={() => setTab('qso')} onSignIn={() => setAuthMode('login')} onPolicy={(kind) => setTab(kind)} /> : tab === 'conduct' ? <PolicyPage kind="conduct" catalog={activeCatalog} onBack={() => setTab('home')} /> : tab === 'rules' ? <PolicyPage kind="rules" catalog={activeCatalog} onBack={() => setTab('home')} /> : <><section className="hero"><div className="hero-copy"><p className="eyebrow">Municipal Parks on the Air</p><h1>{t.hero}</h1><p className="hero-text">{t.heroText}</p><div className="hero-actions"><button className="button" onClick={() => setTab('propose')}>{t.propose}</button><span className="stat"><strong>{parks.length}</strong> {t.approved}</span></div></div></section></>}
      ...
      {tab === 'park-editor' && editingParkId && canModerate && <ParkEditorPage parkId={editingParkId} token={token} t={t} parkTypes={parkTypes} locale={locale} onClose={closeParkEditor} onSaved={() => { setMessage(t.parkChangesSaved); closeParkEditor(); }} />}
      {tab === 'qso-admin' && canGlobalAdmin && <QsoAdminPage token={token} t={t} onMessage={setMessage} />}
      {tab === 'translations' && canGlobalAdmin && <SystemSettingsPage token={token} locale={locale} t={t} settings={systemSettings} onSaved={setSystemSettings} onMessage={setMessage} />}
      ...
      {tab === 'system-settings' && canSystemAdmin && <SystemSettingsPage token={token} locale={locale} t={t} settings={systemSettings} onSaved={setSystemSettings} onMessage={setMessage} />}
    </main>
