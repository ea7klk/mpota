...
function MapView({ parks, picking, selectedPoint, view, t, onPick, onViewChange, onParkSelect, onVisibleParksChange, onViewportChange }: { parks: Park[]; picking: boolean; selectedPoint?: Coordinates; view: MapViewState; t: Record<string, string>; onPick: (lat: number, lon: number) => void; onViewChange: (view: MapViewState) => void; onParkSelect: (park: Park) => void; onVisibleParksChange: (parks: Park[]) => void; onViewportChange?: () => void }) {
  ...
  useEffect(() => {
    ...
    const handleViewportChange = () => { saveView(); updateVisibleParks(); onViewportChangeRef.current?.(); };
...
export default function App() {
  ...
  const refreshParks = () => request<Park[]>('/parks').then(setParks).catch(() => undefined);
  const refresh = () => { void refreshParks(); request<Award[]>('/awards').then(setAwards).catch(() => undefined); if (token) request<User>('/auth/me', {}, token).then(setUser).catch(() => { setToken(''); localStorage.removeItem('mpota-token'); }); };
  ...
  const selectTab = (id: string) => { if (id !== 'park-editor' && editingParkId) { setEditingParkId(null); window.history.pushState({}, '', '/'); } setTab(id); if (id === 'map') void refreshParks(); if (id === 'admin') loadQueue(); if (id === 'users') loadUsers(); if (id !== 'uploads') { setSelectedUploadId(null); setRejectedQsos([]); } };
