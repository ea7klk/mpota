...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      const payload = result.payload as typeof result.payload & { realm_access?: { roles?: string[] }; preferred_username?: string; name?: string; locale?: string };
      const role = payload.realm_access?.roles?.find((candidate) => ['MEMBER', 'ENTITY_ADMIN', 'AWARD_ADMIN', 'GLOBAL_ADMIN', 'SYSTEM_BOOTSTRAP_ADMIN'].includes(candidate)) ?? 'MEMBER';
      request.user = { id: payload.sub!, email: String(payload.email ?? payload.preferred_username ?? ''), displayName: String(payload.name ?? payload.preferred_username ?? ''), role, locale: String(payload.locale ?? 'en') } satisfies AuthUser;
      return true;
    } catch {
